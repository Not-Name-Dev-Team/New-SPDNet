...
import {
  Lock, RefreshRight, ArrowLeft, Connection, Message, Key
...
const handleSendCode = async () => {
  ...
  try {
    const res = await playerApi.sendForgotPasswordCode({ email: form.email })
    if (res.data.success) {
...
const handleResetPassword = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        const res = await playerApi.resetPassword({
          email: form.email,
